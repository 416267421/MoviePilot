...
class BrushFlowLowFreq(_PluginBase):
    ...
    def __filter_torrents_contains_subscribe(torrents: Any, subscribe_titles: Set[str]):
        ...
        for torrent in torrents:
            ...
            # 2026-08-30 本地版：修复单字符别名误杀——订阅「空枪」的 TMDB 单字母别名「V」会子串命中一切含 V 的标题（DV 杜比视界全中招）
            # 长度 <2 的订阅标题（单字母/单字符别名）不参与匹配，直接跳过
            if any(
                len(subscribe_title.strip()) >= 2
                and (subscribe_title in title or subscribe_title in description)
                for subscribe_title in subscribe_titles
            ):
